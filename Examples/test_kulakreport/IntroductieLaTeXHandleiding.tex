...
Wat in wetenschappelijke teksten vaak voorkomt, zijn tabellen met getallen zoals in Tabel \ref{tab:decimalen}. Die worden voor de leesbaarheid zo veel mogelijk op het decimaalteken gealigneerd. Het pakket \pakket{siunitx} voorziet hiertoe de extra kolomdefinitie \code{S}. Deze kolom behandelt alle invoer automatisch net als het \cmd{num}\opt-commando.

\begin{table}
    \centering
    \caption{Verschillende (foute) manieren om getallen te zetten, de correcte manier gebruikt het commando \cmd{num}\opt\ uit het \texttt{siunitx}-pakket.}
    \label{tab:getalnotaties}
