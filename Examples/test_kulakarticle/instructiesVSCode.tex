...
Het document behandelt niet alleen de basisprincipes van \LaTeX\ als tekstzetsysteem (sectie \ref{sec:latex}), maar ook de installatie van de benodigde software (TeX Live en VSCode, sectie \ref{sec:texlive}), het gebruik van KU Leuven-specifieke documentklassen (sectie \ref{sec:huisstijl}) en meer gevorderde functionaliteit in VSCode (spellingscorrectie, custom builds en externe berekeningen; sectie \ref{sec:vscode}) en \LaTeX\ (bibliografiebeheer, figuren en tekeningen; sectie \ref{sec:tips}).
...
Er zijn zeker ook nadelen aan het gebruik van \LaTeX. Het heeft een steilere leercurve, de software is niet zo eenvoudig te installeren, maar vooral: het is zeer moeilijk om er lelijke documenten mee te maken.
...
\begin{description}
	\item[Editor.] De tekst inclusief markup-commando's wordt getypt in een \texttt{.tex}-bestand. Eender welke editor volstaat (bijvoorbeeld Kladblok) maar om het vlotst te kunnen werken is het gebruik van een editor die speciaal ontworpen is voor \LaTeX\ aanbevolen. Verderop wordt gebruik van \textbf{VSCode} toegelicht, maar vergelijkbare functionaliteit is te vinden in elke actuele editor zoals TeXstudio of Overleaf (online, zeer geschikt om samen te werken met meerdere auteurs).
...
\subsection{\LaTeX-editor: VSCode}
Zorg dat de \LaTeX-distributie eerst is afgewerkt vooraleer te beginnen met het installeren van VSCode voor Windows.

\begin{enumerate}
	\item Surf naar de downloadpagina van VSCode, \href{code.visualstudio.com/}{\texttt{code.visualstudio.com}}.
	\item Klik je op de downloadloadknop die hoort bij je besturingssysteem
	\item Start na het downloaden de installer.
	\item Aanvaard de voorwaarden, de standaard instellingen zijn ok, klik op ``Install''.
	\item Installeer vervolgens de extensie \texttt{LaTeX Workshop} (\texttt{ctrl+shift+X}).
	\item Specifieke commando's voor deze extensie zijn te vinden in de \TeX-zijbalk (\texttt{ctrl+alt+X}).
\end{enumerate}
...
Volg de instructies in één van volgende secties om de class-files te installeren.
\begin{itemize}
	\item \textbf{Downloaden en configureren} (\ref{sssec:dl}) voor wie GitHub niet kent, je zal nieuwe versies van de huisstijl telkens manueel moeten installeren.
	\item \textbf{Repository clonen en symlinks maken} (\ref{sssec:gh}) voor de ervaren Github gebruiker, je werkt steeds met de recentste versie van de huisstijl.
	\item \textbf{Linux en macOS gebruikers} (\ref{sssec:linux} en \ref{sssec:macos}) volgen dezelfde principes, al zijn de bestandslocaties verschillend. Deze worden verderop gespecificeerd.
\end{itemize}
Na installatie van de class-files, kan je \texttt{VSCode} configureren om de templates te gebruiken (\ref{sssec:templates}).
...
\paragraph{Class files configureren in Windows.}
\begin{enumerate}[resume]
...
\subsubsection{Class files configureren in Linux.}
\label{sssec:linux}
...
\subsubsection{Class files configureren in MacOS.}
\label{sssec:macos}

De werkwijze op macOS is analoog als die op Windows, met volgende verschillen.
\begin{itemize}
	\item Map \texttt{kulak} verplaatsen naar \verb+~/Library/texmf/tex/LaTeX/+
	\item Of een symbolische link maken naar de geclonede repository met onderstaand commando
	      \begin{verbatim} ln -s <pad_naar_repo>/KULeuvenLaTeXClasses/kulak ~/Library/texmf/tex/latex/kulak\end{verbatim}
\end{itemize}


\subsubsection{Templates configureren in VSCode.}
\label{sssec:templates}
\begin{enumerate}
	\item De meegeleverde Kulak-templates zijn te vinden in de map \texttt{Templates} van het pakket \texttt{KULeuvenLaTeXClasses}. Noteer de locatie van deze map, bijvoorbeeld
	\begin{verbatim}C:\Projecten\KULeuvenLaTeXClasses\Templates
	\end{verbatim}
	\item Installeer de extensie \texttt{Template Generator} in VSCode (\texttt{ctrl+shift+X}). Klik na installatie op \texttt{Manage} (tandwiel), \texttt{Settings} en plak de locatie van de template-map in het veld \texttt{Templates: Path}.
	\item Een nieuw bestand openen volgens een template kan in \texttt{Explorer} (\texttt{ctrl+shift+E}) door in de gewenste map te rechtsklikken en te kiezen voor \texttt{Generate Template}. Selecteer het gewenste template.
\end{enumerate}

\subsection{Specifieke commando's en opties}
\subsubsection{Logo's}
De vier documentklassen hebben telkens de opties \texttt{kulak}, \texttt{kuleuven-brugge}, en \texttt{kul} om het juiste logo te selecteren. De optie \texttt{ingenieurswetenschappen} is enkel beschikbaar voor de \texttt{article} documentklasse.
\begin{center}
	\begin{tabular}{c c}
...
\subsubsection{Huisstijl kleurenpallet}

De kleuren die horen bij de KU Leuven huisstijl zijn opgelijst in onderstaande tabel en beschikbaar met het commando \verb+\textcolor{naam}{tekst}+.

~\hfill\begin{tabular}{ll|r|r|r}
	                            & &   R &   G &   B \\\hline
	KULijsblauw &\textcolor{KULijsblauw} {\rule{1ex}{1ex}}   & 220 & 231 & 240 \\
	KULwit      &\textcolor{KULwit}    {\rule{1ex}{1ex}}           & 255 & 255 & 255 \\
	KULcorporate&\textcolor{KULcorporate}{\rule{1ex}{1ex}} &   0 &  64 & 122 \\
	KULpetrol   &\textcolor{KULpetrol}   {\rule{1ex}{1ex}}       &  20 & 127 & 161 \\
	KULzwart  & \textcolor{KULzwart} {\rule{1ex}{1ex}} &   0 &  29 & 65  
\end{tabular} \hfill
\begin{tabular}{ll|r|r|r}
	          &                                       &   R &   G & B   \\ \hline

	KULrood   & \textcolor{KULrood}  {\rule{1ex}{1ex}} & 240 & 119 & 110 \\
	KULoranje & \textcolor{KULoranje}{\rule{1ex}{1ex}} & 251 & 176 & 55  \\
	KULgeel   & \textcolor{KULgeel}  {\rule{1ex}{1ex}} & 228 & 218 & 62  \\
	KULgroen  & \textcolor{KULgroen} {\rule{1ex}{1ex}} & 135 & 192 & 189 \\
	KULblauw  & \textcolor{KULblauw} {\rule{1ex}{1ex}} &  82 & 189 & 236 \\
	KULpaars  & \textcolor{KULpaars} {\rule{1ex}{1ex}} & 199 & 147 & 174
\end{tabular}\hfill~
...
Goede werking vereist volgende commando's in de preambule:
\begin{itemize}
...
\end{itemize}
Met het optionele \verb+\subtitle{}+ kan ook een (cursieve) subtitel worden toegevoegd onder de hoofdtitel.
Om de naam van de instelling (automatisch) en de afdeling (via \verb+\department{}+) te verbergen uit de rechterbovenhoek van pagina's (verschillend van de eerste), kan het commando \verb+\hideDetails+ worden gebruikt.
...
\section{Gebruik van VSCode en \LaTeX}
\label{sec:vscode}

%\subsection{\LaTeX\ in VSCode}
Een bestand met de extensie \texttt{.tex} activeert automatisch de \texttt{LaTeX Workshop}-extensie van \texttt{VSCode}. Dit zijn de belangrijkste functies van deze extensie:
\begin{itemize}
	\item \texttt{TeX}-knop in de zijbalk (\texttt{ctrl+alt+X}) toont de titelstructuur van het document en een hele resem commando's, zoals builden, bekijken, hulpfiles opschonen, \ldots
	\item \texttt{Build LaTeX-project} (groene pijltje in de knoppenbalk, \texttt{ctrl+alt+B}) genereert het uitvoerbestand (\texttt{.pdf}).
	\item \texttt{View LaTeX PDF File} (knoppenbalk, \texttt{ctrl+alt+V}) opent de ingebouwde PDF-viewer.
	\item Forward search: spring van code naar de overeenkomstige plek in de \texttt{.pdf} (\texttt{ctrl+alt+J}).
	\item Inverse search: spring van de \texttt{.pdf} naar de overeenkomstige plek in de \texttt{.tex}-code (\texttt{ctrl+click}).
	\item Het \texttt{Problems panel}  inventariseert alle \LaTeX- en spellingsfouten in het document (klik links onderaan op het waar/schuwingsicoontje of \texttt{ctrl+shift+M}). Klikken op een probleem springt naar de de regel in de bron waar het probleem is vastgesteld (zoals steeds in \LaTeX\ niet noodzakelijk de plek waar het probleem kan worden opgelost).
	\item Een hele resem extra commando's in de quick access balk (\texttt{ctrl+shift+P}, typ ``LaTeX'').
\end{itemize}

Nog wat handige trucjes:
\begin{itemize}
	\item Toggle comment (\texttt{ctrl+/}) zet de geselecteerde tekst om in een commentaar of haalt het commentaar weg.
	\item Toggle word wrap (\texttt{alt+Z}) laat tekstregels automatisch afbreken aan de rand van het editorvenster (zoals in een tekstverwerker) of zet de tekst in de editor op één lijn (zoals gebruikelijk bij programmacode).
	\item Multi cursor (\texttt{alt+(shift+)click}) zet de cursus op meerdere plaatsen tegelijk, zodat je meerdere lijnen tegelijk kan aanpassen (bijvoorbeeld nuttig bij het maken van tabellen).
	\item Command palette (\texttt{ctrl+shift+P}) geeft zoals steeds in \texttt{VSCode} toegang tot alle commando's en de extensies. Typ ``\texttt{LaTeX}'' om meteen de relevante commando's te zien.
\end{itemize}
% \subsection{Extra recept in VSCode: slides en handouts}
% Met een extra recept van de extensie \texttt{LaTeX Workshop} kan één build zowel de gewone presentatie als de handouts genereren. Open de werkruimte-instellingen via ``Preferences'', ``Settings'', klik rechtsboven op ``Open Settings (JSON)'' en voeg volgende instellingen toe. Bestaan de velden al, voeg dan enkel de nieuwe tools en recipe toe aan de bestaande lijsten.
% \begin{verbatim}
% {
% 	"latex-workshop.latex.tools": [
% 		{
% 			"name": "pdflatex-slides",
% 			"command": "pdflatex",
% 			"args": [
% 				"-synctex=1",
...
% 		},
% 		{
% 			"name": "pdflatex-handout",
% 			"command": "pdflatex",
% 			"args": [
% 				"-jobname=%DOC%-handout",
% 				"-interaction=nonstopmode",
% 				"-file-line-error",
% 				"\\PassOptionsToClass{handout}{beamer}\\input{%DOC%}"
% 			]
% 		}
% 	],
...
% }
% \end{verbatim}
% De eerste tool maakt de gewone presentatie \texttt{file.pdf}; de tweede geeft de optie \texttt{handout} mee zonder het bronbestand aan te passen en maakt \texttt{file-handout.pdf}. Selecteer daarna in de \TeX-zijbalk de recipe ``Slides + handout'' of gebruik het commando ``LaTeX Workshop: Build with recipe'' uit de Command Palette (\texttt{ctrl+shift+P}).
